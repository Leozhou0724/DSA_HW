/*
This is from wikipedia Chinese version
https://zh.wikipedia.org/wiki/%E7%BA%A2%E9%BB%91%E6%A0%91#C++%E7%A4%BA%E4%BE%8B%E4%BB%A3%E7%A0%81

Changes I made:

Line 440-487:
Add an function int length(), find the node and return the length

Line 420-436:
Add a public int redn in the class bst to count the number of red nodes
Add two functions void coutrend() and void traversecount() to traverse all the nodes and count the number of red nodes
set redn=0 in the constructor of this class
*/
...
class bst
{
  ...
  public:
    ...
    void traversecount(Node *ptr)
    {
        ...
        traversecount(ptr->leftTree);
        traversecount(ptr->rightTree);
    }
    void countredn()
    {
        Node *ptr = root;
    ...
    int length(int n)
    {
        ...
        while (temp != NULL)
        {
            if (n < temp->value)
            {
                ...
                {
                    temp = temp->leftTree;
                    if (temp->color != 0) //length added if the node is not red node
                    {
                        L++;
                    }
                }
